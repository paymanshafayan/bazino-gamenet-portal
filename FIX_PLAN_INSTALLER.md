...
## نتیجه‌ی اصلاح Web Sync — ۱۴۰۵/۰۶/۱۲

مشکل گزارش‌شده تکمیل شد: پنل ادمین اکنون در بخش API Keys یک کارت مستقل برای ساخت و ذخیره‌ی `gamenet_sync_api_key` دارد. مقدار واقعی فقط پس از generate/save برمی‌گردد تا مدیر آن را در برنامه دسکتاپ کپی کند؛ خواندن بعدی فقط masked است و `/api/settings` عمومی همچنان secret را حذف می‌کند. دو endpoint ادمین و تست‌های masking/access-control اضافه شد. تست نهایی: **243/243**.
