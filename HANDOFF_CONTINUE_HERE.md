...
> **بازتأیید جدید تحویل V4 — 2026-09-07:** کد هشت بچ قبلاً روی شاخهٔ همین نشست و PR #20 است؛ فایل‌های workspace با `660fd9a` تطبیق داده شدند و نیاز به ساخت دوباره ندارند. اکنون TypeScript/build و 506/506 تست مجدداً اجرا و موفق شده‌اند؛ Chromium پس از یک timeout اولیه، در تکرار مستقل گذشت و همهٔ ۲۰ تصویر تازه واقعاً بازبینی شدند. dispatch استقرار همچنان 403 است و health دامنه HTML است، نه JSON receiver. بخش «بازتأیید تحویل» در `docs/publishing/V4_DELIVERY.md` و ZIP شواهد مرجع‌اند؛ موفقیت محلی را با انتشار زنده یکی ندانید.
