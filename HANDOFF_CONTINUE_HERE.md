...
## ★ وضعیت جدیدتر — 2026-09-07: متن کامل طرح افیلیت و پلن Media-ID-only

**ابتدا این بخش و `HANDOFF_PROMPT.md §۱۹` خوانده شود.** متن کامل کاربر از پیام دریافت و در `docs/payments/INSTAGRAM_AFFILIATE_DESIGN.md` ذخیره شد؛ مانع پیوست قبلی رفع شده و درخواست تکرار سند لازم نیست. پلن نهایی پیشنهادی در `docs/payments/INSTAGRAM_MEDIA_ONLY_PLAN.md` است؛ **هنوز اجرای کد/استقرار تأیید یا شروع نشده**.

- Manus فقط ID رسانهٔ منتشرشده را به `published-media` می‌فرستد؛ backend آن را با Media ID رویداد Zernio تطبیق می‌دهد. حساب متصل/کمپین نیز باید مجاز باشند.
