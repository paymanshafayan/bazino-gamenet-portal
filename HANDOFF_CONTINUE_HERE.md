...
## ★ وضعیت جدیدتر — 2026-09-07: پلن نسخهٔ ۴، سوییچ دستی/عامل و Manus پیش‌فرض

**ابتدا این بخش و `HANDOFF_PROMPT.md §۲۲` خوانده شود.** اصل متن کاربر در `docs/payments/INSTAGRAM_AFFILIATE_DESIGN.md` محفوظ است. پلن `docs/payments/INSTAGRAM_MEDIA_ONLY_PLAN.md` اکنون **نسخهٔ ۴ / ۸ بچ** است؛ Composer در معماری §۵٫۱ و سوییچ/Agent Registry در §۵٫۲ آمده‌اند. **هنوز اجرای کد/استقرار تأیید یا شروع نشده**.

- امروز Manus فقط ID رسانه را می‌دهد؛ در آینده پنل از API نشر Zernio استفاده می‌کند. هر دو به registry رسانهٔ مصوب مشترک می‌رسند و affiliate به نام ناشر وابسته نیست. منبع اکتشاف external به‌تنهایی مجوز کمپین نمی‌دهد.
- درخواست جدید ادمین: Post Composer با upload واقعی تصویر/ویدئو، کپشن، تک‌تصویر/Carousel/Reel و Story مجاز، ترتیب اسلاید/preview/کاور، فایل ماندگار و انتشار اکنون/زمان‌بندی؛ در بچ ۷ صریحاً قرار گرفت. فرم متن و mediaUrl تک‌فایل فعلی این قابلیت نیست؛ فقط طراحی انجام شده است.
- افزودهٔ جدید: mode دستی/عامل و default ماندگار، تعریف عامل با نام/type/API Key، Manus built-inِ ذخیره‌شده در DB؛ بدون کلید آماده معرفی نشود و seed انتخاب مدیر را reset نکند. کلید عامل با توکن ingest متفاوت است؛ منبع فعلی کلید Manus در مهاجرت حفظ و پاک‌کردن کلید با fallback پنهان خنثی نشود.
- همکار فقط PR توضیحی و سپس DM کد/متن، **بدون لینک دعوت**؛ دوست پس از کامنت کد زیر همان رسانه و Gate، لینک را در DM می‌گیرد. چهار کلیدواژه/زبان مستقل و یک PR برای هر کامنت.
- URL اعلام‌شدهٔ کاربر ثابت: `https://bazino.pro/api/webhooks/zernio`؛ کنار comment.received، message.received برای دکمه لازم است. اشتراک سمت Zernio آماده اعلام شده؛ receiver در کد/هاست هنوز پیاده/تأیید نشده است.
- پیشنهاد وب‌هوک اصلی ۲۳ event مربوط به نشر/حساب/تعامل است؛ `analytics.synced` در اشتراک و صف جدا، با endpoint برنامه‌ریزی‌شده ولی هنوز مستقرنشده. فهرست دقیق انتخاب/عدم انتخاب در سند جدید است. ساخت subscription/انتشار یا تغییر Secret این نوبت انجام نشد.
- بخش‌های قدیمی پایین که Manus را مسئول `partner-invite`/فرستادن لینک به شریک می‌دانند، **تاریخچهٔ کد موجودند، نه قرارداد هدف جدید**. ZIP ورودی نیز عیناً deploy نمی‌شود.
- قانون ۳ هندآف: بعد از هر تغییر کد یا اجرای پلن/batch، اسناد مرتبط + هندآف خودکار به‌روز و تغییرات مرتبط روی شاخهٔ جلسه commit/push شوند. قانون ۴: سطح اثبات صادقانه و لینک خصوصی فقط برای دوست.
- شاخهٔ این نشست فقط `arena/01a07a2f-bazino-gamenet-portal` است؛ شاخه‌های قدیمی پایین مقصد کار نیستند. کد محصول این نوبت تغییر نکرد؛ نتایج تست‌های قبلی شاهد تست این پلن نیستند.
