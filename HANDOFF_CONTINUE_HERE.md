...
> ★ **وضعیت جاری — 2026-09-08 (§۲۵):** فهرست ۷موردی کارفرما **پیاده‌سازی و تحویل شد** — چت فقط غیرفعال (از منو حذف، کد سالم + پرچم `chat_enabled`)، پیامک دست‌نخورده، **کردیت بازینو (BC) با خرج‌کردن کامل** (پرداخت رزرو با BC + `creditPrice` کاتالوگ + شارژ ادمین)، رفع باگ «مدیریت مشتریان» (E.131)، **دستهٔ اضافه فقط کنسول** (گارد سرور)، کافه/فروشگاه coming-soon با مسیر سفارش سالم (پرچم‌های `food/shop_coming_soon`). آزمون: unit **۱۱۶/۱۱۶**، providers ۲۷/۲۷، ui ۴۵/۴۵، tsc/build سبز، ZIP هاب ری‌بیلد و راستی‌آزمایی شد. کامیت `0de69f4` روی `arena/01a0800f` (پوش شد)؛ PR به `main` در همین پیگیری باز/مرج می‌شود. **کار بعدی:** رگرسیون زنده روی محیط واقعی (پرداخت BC، استپر دسته، پرچم‌ها، شارژ ادمین) پیش از اعلام به کارفرما. جزئیات: `HANDOFF_PROMPT.md §۲۵`.
>
> **وضعیت قبلی — 2026-09-07:** بخش **۲۳ هندآف** و `docs/publishing/V4_DELIVERY.md` مرجع‌اند. کد هشت بچ در شاخه و PR #20 موجود است؛ بازاجرای واقعی روی `45dd2ad` شامل TypeScript/build، 506/506 تست و Chromium موفق در اولین تلاش با ۲۰ تصویر بازبینی‌شده و restart بود. فقط اسناد این پیگیری تغییر می‌کنند. آخرین dispatch استقرار 403 و health وب HTML است؛ کار اصلی باقی‌مانده، مجوز/استقرار و تأیید live مجاز است، نه ساخت دوبارهٔ امکانات.


## ★ وضعیت جاری — 2026-09-07: اجرای V4 و رگرسیون نهایی

هشت بچ کد و تست محلی اجرا شدند؛ **506/506** تست، TypeScript/build، Chromium واقعی چهارزبانه و restart فرایند با حفظ رسانه موفق‌اند. مرجع اصلی [گزارش تحویل](docs/publishing/V4_DELIVERY.md) و [راهنمای هاست/استفاده](docs/publishing/DEPLOY_AND_OPERATE.md) است. این دیگر صرفاً پلن نیست.

- وب‌هوک HMAC، registry فقط رسانهٔ مصوب، فلو کد-فقط همکار/لینک-فقط دوست، Gate واقعی، کوپن و مالی مرجع پیاده شدند.
- استودیوی مشترک سایت/مدیریت، upload/Composer و آداپتور دستی/عامل، Manus پیش‌فرضِ ماندگار و کلیدهای سروری رمزگذاری‌شده وجود دارند.
- API خارجی در تست‌ها mock است؛ روی حساب واقعی هیچ پیام/پست/اعتبار یا پولی مصرف نشد. SQL Server/Mongo واقعی تست نشده‌اند.
- ۲۰ تصویر کامل تولید شد؛ بازبینی تصویر، فونت سیریلیک گم‌شده را پیدا کرد؛ اصلاح و اجرای دوباره انجام شد. DOM-only به‌عنوان بازبینی تصویر گزارش نشد.
- مشکل قبلی push بچ‌های ۵–۷ در این نوبت رفع و ارسال آن‌ها موفق شد. آخرین PR/کامیت/نتیجهٔ استقرار را از گزارش تحویل بخوانید؛ داشتن webhook در Zernio یا سبزبودن build، اثبات انتشار روی دامنه نیست.
- فقط روی `arena/01a07a2f-bazino-gamenet-portal` کار/push شود؛ سکرت‌ها در چت/Git، ZIP خروجی در Git، SMTP/پرداخت آنلاین/POS سخت‌افزاری فعال نشوند. منابع و قواعد طراحی پایین تاریخی‌اند.
