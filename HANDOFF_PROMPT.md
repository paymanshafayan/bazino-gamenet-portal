...
> سوابق اجرا و تست هر بخش مربوط به همان نشست‌اند؛ نتیجهٔ تحقیق، موارد اجراشده و موارد هنوز آزمایش‌نشده باید از هم تفکیک شوند. وضعیت فعلی در **بخش ۱۵** است؛ خلاصه: batchهای ۱–۵ پلن مدیریت اجرا و با PR #18 در `main` ادغام شدند، batchهای ۶–۸ باقی‌اند و پروندهٔ سخت‌افزار POS فعلاً کنار گذاشته شده است.
>
> تاریخ تنظیم: ۱۴۰۵/۰۶/۱۰ (2026-09-01) · آخرین به‌روزرسانی: ۱۴۰۵/۰۶/۱۵ (2026-09-06، بازبینی کد پس از اجرا: وضعیت واقعی batchهای ۱–۵ و باقی‌ماندهٔ ۶–۸ در **بخش ۱۵**) · ریپو: `paymanshafayan/bazino-gamenet-portal`
> برنچ نشست قبل: **`arena/01a070af-bazino-gamenet-portal`** (۵ کامیت اجرای پلن، HEAD `3fd8a7d`) که با **PR #18 در `main` ادغام شد** (merge commit `81f9b84`). برنچ نشست جاری `arena/01a07603-bazino-gamenet-portal` پس از merge، **از روی `main` تازه بازسازی شده** (پایه `81f9b84`) و فقط به‌روزرسانی همین هندآف را روی خود دارد. تغییرات نشست `arena/01a06e3e-…` با PR #17 در `main` ادغام شده‌اند.
...
>
> **اول بخش ۱۵ را بخوانید** (وضعیت واقعی کد پس از اجرای پلن مدیریت). خلاصه: batchهای ۱–۵ (رزرو روی ایستگاه، کافه/فروشگاه، افیلیت+کیف پول، نقدی/POS) در کد کامل‌اند؛ batchهای ۶ (کوپن/ساعات ویژه)، ۷ (تورنومنت) و ۸ (محتوا/Manus) فقط پلن و رزرو تایپ/مجوز دارند و **کد نشده‌اند**. POS سخت‌افزاری و تستر آن متوقف‌اند. مرجع: `docs/management/EXPANSION_PLAN.md` و `docs/management/EXECUTION_LOG.md`.
...
7. در هر نشست فقط روی **برنچ ردیابی‌شدهٔ همان نشست** کار و push شود (نشست جاری: **`arena/01a07603-bazino-gamenet-portal`** که از `main` پس از PR #18 بازسازی شده)؛ شاخه‌های تاریخی (`arena/01a070af-…` و قبل‌تر) مقصد کار نیستند و روی `main` مستقیماً کار یا push نشود — خروجی هر نشست با PR در `main` ادغام می‌شود.
...
## ۱۵. بازبینی کد پس از اجرا — چه چیزی واقعاً ساخته شد و چه چیزی باقی است
