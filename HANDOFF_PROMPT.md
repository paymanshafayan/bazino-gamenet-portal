...
>
> تاریخ تنظیم: ۱۴۰۵/۰۶/۱۰ (2026-09-01) · آخرین به‌روزرسانی: **۱۴۰۵/۰۶/۱۶ (2026-09-07)**، بازگشت نقش Manus به Media-ID-only، وضعیت پیوست جدید و قانون تحویل خودکار در **بخش ۱۸ و قانون ۳**؛ گزارش پس از PR #19 در بخش ۱۶ و سابقهٔ ZIP در بخش ۱۷ · ریپو: `paymanshafayan/bazino-gamenet-portal`
...
>
> **کار جاری: بخش ۱۸ و `docs/payments/INSTAGRAM_MEDIA_ONLY_PLAN.md`.** کاربر خواسته Manus فقط ID پست/ریل منتشرشده را بفرستد و بقیهٔ افیلیت در بک‌اند انجام شود. **اصل پیوست جدید در workspace پیدا نشده؛ پیش‌نویس فعلی فقط از پیام و کد موجود است، نه از سند خوانده‌شده.** کاربر ساخت اشتراک Zernio روی `/api/webhooks/zernio` را اعلام کرده؛ استقرار گیرنده تأیید نشده و کد تغییر نکرده است. قانون ۳: اسناد + commit/push خودکار، بدون درخواست جداگانه.
>
> **اول بخش ۱۶ را بخوانید.** بخش ۱۵، بازبینی تاریخیِ پایهٔ PR #18 است و عبارت‌های «batchهای ۶–۸ کد نشده‌اند» در آن دیگر وضعیت فعلی نیستند. وجود کد/رابط با تکمیل همهٔ معیارهای پذیرش، سبزبودن کل تست‌ها یا تأیید اتصال زنده یکی نیست؛ مرز گزارش و نکات باز در §۱۶٫۳ آمده‌اند. POS سخت‌افزاری و تستر آن متوقف‌اند. مراجع: `docs/management/EXPANSION_PLAN.md`، `docs/management/EXECUTION_LOG.md` و بخش ستاره‌دار بالای `HANDOFF_CONTINUE_HERE.md`.
...
7. در هر نشست فقط روی **برنچ ردیابی‌شدهٔ همان نشست** کار و push شود (نشست جاری: **`arena/01a07a2f-bazino-gamenet-portal`**، پایهٔ `47021330` پس از PR #19)؛ شاخه‌های تاریخی (`arena/01a07603-…`، `arena/01a070af-…` و قبل‌تر) مقصد کار نیستند و روی `main` مستقیماً کار یا push نشود — خروجی هر نشست با PR در `main` ادغام می‌شود.
...
## ۱۸. بازگشت به Media-ID-only — قانون تحویل خودکار ثبت شد؛ دریافت اصل سند مسدود است
