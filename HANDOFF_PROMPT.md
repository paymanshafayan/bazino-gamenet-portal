...
- **✅ زنجیرهٔ تلگرام کامل شد (شب ۲۰۲۶-۰۹-۱۰، 17:18Z):** کارفرما مراسم کنسولی را خودش انجام داد (send+login در Console؛ رشته در `TG_SESSION_STRING`؛ اولین تست PowerShell اشتباهاً سشن را به‌جای Bearer فرستاد → جدول تشخیص طول داده شد) و راستی‌آزمایی نهایی از PowerShell خودش: `/internal/health` → `ok=True`، **`session_configured=True`، `fake_mode=False`** — یعنی اداپتر واقعی Telethon در استارتاپ وصل شده (سشن خراب بود crash-loop می‌شد و JSON نمی‌آمد). **جاماندهٔ عملکردی (تست‌نشده):** یک send واقعی — پیشنهاد: اول dialogs از پنل ادمین (Manus/تلگرام، بدون ریسک، باید دیالوگ‌های واقعی اکانت بازینو را نشان دهد)، بعد DM آزمایشی. بقیهٔ بک‌لاگ پابرجا: هاب bazino.pro، webhook.test/INVALID_SIGNATURE، فلو دوست بخش دوم، رگرسیون §۲۵.
- **ری‌ست سندباکس (شب):** `/home/user/cdp` (پل CDP) و `/tmp/tg-secrets4.txt` پاک شدند — پل **پایین** است؛ برای هر اتوماسیون UI ریلوی باید طبق `docs/ops/CDP_BROWSER_BRIDGE.md` بازسازی و کارفرما اسکریپت PowerShell را با URL جدید اجرا کند. راستی‌آزمایی گیت‌وی/پورتال بدون پل هم ممکن است: مرورگر خود کارفرما (healthz) + `Invoke-RestMethod` پاورشل با GATEWAY_BEARER از پنل.
