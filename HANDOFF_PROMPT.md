...
- **ری‌ست سندباکس (شب):** `/home/user/cdp` (پل CDP) و `/tmp/tg-secrets4.txt` پاک شدند — پل **پایین** است؛ برای هر اتوماسیون UI ریلوی باید طبق `docs/ops/CDP_BROWSER_BRIDGE.md` بازسازی و کارفرما اسکریپت PowerShell را با URL جدید اجرا کند. تا آن موقع راستی‌آزمایی گیت‌وی/پورتال فقط از مرورگر خود کارفرما (healthz در تب مرورگر + `Invoke-RestMethod` پاورشل با GATEWAY_BEARER از پنل).
