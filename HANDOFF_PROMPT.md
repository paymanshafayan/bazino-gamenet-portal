...
>
> تاریخ تنظیم: ۱۴۰۵/۰۶/۱۰ (2026-09-01) · آخرین به‌روزرسانی: ۱۴۰۵/۰۶/۱۴ (2026-09-05، پایان نشست IG+هندآف) · ریپو: `paymanshafayan/bazino-gamenet-portal`
> برنچ فعلی جلسه: **`arena/01a06e3e-bazino-gamenet-portal`** (HEAD باید `4dfb6ed` یا بعد از آن باشد). برنچ قدیمی `arena/01a067ac-…` در `main` ادغام شده (PR #16).
>
> **اول بخش ۹ («این نشست — افیلیت، اینستاگرام، POS ناتمام») را بخوانید** — کار بعدی همان‌جاست. سپس بخش ۸ برای تاریخچه.
...
7. فقط روی برنچ **`arena/01a06e3e-bazino-gamenet-portal`** کار و push شود (نه `01a067ac`، نه `main`).
