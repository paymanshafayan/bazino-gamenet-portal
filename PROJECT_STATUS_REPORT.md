...
> برنچ کاری: `arena/01a067ac-bazino-gamenet-portal` (HEAD: `a403c2d`)
> تاریخ گزارش: ۲۰۲۶-۰۹-۰۱ — **به‌روزرسانی ۲۰۲۶-۰۹-۰۵** (بخش «۰») — **به‌روزرسانی ۲۰۲۶-۰۹-۰۸** (بخش «۰٫۲»؛ برنچ `arena/01a0800f`، HEAD: `0de69f4`)

## ۰٫۲. تغییرات ۲۰۲۶-۰۹-۰۸ — فهرست ۷موردی کارفرما (جزئیات در `HANDOFF_PROMPT.md §۲۵`)

| حوزه | وضعیت |
|---|---|
| چت | فقط غیرفعال (از منوی سایت و هاب حذف؛ کد/روت‌ها سالم؛ پرچم بازگشت `chat_enabled`) ✅ |
| پیامک | بدون تغییر، طبق دستور ✅ |
| کردیت بازینو (BC) | خرج‌کردن کامل: پرداخت رزرو با `POST /api/checkout/credits` + `creditPrice` کاتالوگ + شارژ ادمین + بازگشت در لغو ✅ |
| باگ مدیریت مشتریان | E.131 رفع شد: رفرش پس از mutation از GET ادغام‌شده در هر ۱۰ نقطه ✅ |
| دستهٔ اضافه | فقط کنسول (PS5/Xbox) با گارد سرور `EXTRA_CONTROLLERS_CONSOLE_ONLY` ✅ |
| کافه/فروشگاه | فقط COMING SOON (پرچم‌های `food/shop_coming_soon`) با مسیر سفارش سالم ✅ |
| هاب | ZIP ری‌بیلد و راستی‌آزمایی شد (حذف چت + استپر EXTRA PADS) ✅ |
| تست‌های اجراشده | unit ۱۱۶/۱۱۶ · providers ۲۷/۲۷ · ui ۴۵/۴۵ · tsc/build سبز؛ سوئیت‌های native (api/management/db) در sandbox اجرا نشدند |
| باقی‌مانده | رگرسیون زنده روی محیط واقعی + PR/مرج به `main` |

## ۰. تغییرات از ۲۰۲۶-۰۹-۰۱ تا ۲۰۲۶-۰۹-۰۵ (خلاصه؛ جزئیات در `HANDOFF_PROMPT.md §۸` و `SESSION_SUMMARY.md`)

| حوزه | وضعیت |
|---|---|
| چندزبانه | GeoIP → زبان پیش‌فرض؛ ترجمه‌ی کامل ru/tr؛ پیام‌های خطای API محلی‌شده ✅ |
| قالب‌ها | SDK v2 (regionها، توکن‌ها، رشته‌ها)، نصب اتمیک، پوشه‌ی داده‌ی ماندگار، انتخاب قالب فقط توسط ادمین، `SDK.LocationFrame`، رفع ZIP با پوشه‌ی ریشه ✅ |
| مسیرها | URL routing برای تب‌ها/ادمین/پروفایل/قانونی؛ جست‌وجوی بخش‌های ادمین ✅ |
| احراز هویت | ورود با کد پیامکی (smsto/easysendsms/mock)، محدودیت سمت سرور، رمز دائمی اختیاری ✅ |
| پروفایل و تیکت | `/profile/*` (کیف پول، امتیاز، رزرو، سفارش، تورنمنت، پشتیبانی، امنیت)؛ تیکت کاربر + ادمین با بستن خودکار ۴۸ ساعته ✅ |
| پرداخت | صفحات قانونی/تماس مستقل از قالب، TL؛ PayTR پیاده‌سازی‌شده ولی **خاموش** (`PAYMENT_ONLINE_ENABLED`)؛ **کیف پول + پرداخت در محل** (مهلت ۱۰ دقیقه/۴۸ ساعت، ابطال خودکار؛ بوفه/فروشگاه فقط در محل) ✅ |
| اپ مدیریت | همگام‌سازی کیف پول با سایت (صف آفلاین + idempotency) و تأیید پرداخت‌های حضوری ✅ |
| تست‌ها | `npm test` **۳۵۴/۳۵۴** (unit/db/providers/api/ui) + بازبینی Chromium EN/TR دسکتاپ+موبایل |
| تست‌نشده | SQL Server/Mongo واقعی، Flutter/Electron در sandbox، GTmetrix واقعی، ایمیل (متوقف به دستور کاربر) |
