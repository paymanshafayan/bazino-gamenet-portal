...
## ۰. قواعد ماندگار ارائهٔ پل به کارفرما (ثبت ۲۰۲۶-۰۹-۱۳ — درخواست صریح مکرر)

1. **اسکریپت پل را همیشه به صورت کامل بده** — هرگز «فقط دو خط عوض شود» نگو. نسخهٔ کامل و آماده:
   `cdp-tools/bridge-full.ps1` (با placeholder)؛ ایجنت هر بار `$Base` و `$Code` جلسه را در همان پر می‌کند.
2. **کروم ایجنت مستقل** (پروفایل `chrome-agent` با `--user-data-dir` جدا) — **پروسۀ کروم اصلی کارفرما هرگز بسته یا ری‌استارت نشود** (چت Arena داخل آن است).
3. بعد از ری‌بیلد سندباکس فقط `$Base` عوض می‌شود (شناسهٔ جدید)؛ `$Code` پایدار است و ایجنت فایل `.session-code` را بازسازی می‌کند. رله با `node relay.js` در `cdp-tools/` دوباره بالا می‌آید و باید اولین پورتِ در حال گوش‌دادن باشد.
4. پیام‌های کنسول اسکریپت انگلیسی باشند (قاعدۀ ماندگار PowerShell کارفرما) و دستورهای کاربر همیشه PowerShell-native.
5. **(درس ۲۰۲۶-۰۹-۱۳ عصر — اثبات با fetch_page)** دامنهٔ `{port}-{id}.e2b.app` الان هدر `e2b-traffic-access-token` را الزامی می‌کند («The sandbox … enforces traffic access token») که فقط مرورگر خود کارفرما (جلسهٔ Arena) دارد ⇒ **PowerShell هرگز از e2b.app عبور نمی‌کند (403/502)**. مسیر پل همان `sbx-<id>.arena.site` است؛ اگر 403/502 داد اول وضعیت VPN کارفرما را چرخش بده و حلقهٔ pre-check را دوباره اجرا کن (هم با UA پیش‌فرض هم با UA مرورگر — Invoke-RestMethod با `-UserAgent`). منبع نهایی Base: URL نوار آدرسِ پیش‌نمایش زندهٔ «CDP Browser Bridge Relay» در مرورگر خود کارفرما.
6. **(درس ۲۰۲۶-۰۹-۱۳ عصر ۲ — ثبت قطعی)** **نام میزبان عمومی پیش‌نمایش هیچ‌وقت از `E2B_SANDBOX_ID` حدس زده نشود** — این دو با هم فرق دارند (مثال این جلسه: sandbox id = `i6kbsthngkkzntvgolcy2` ولی میزبان واقعی = `sbx-pfz36v42usj4yd6v.arena.site`). حدسِ اشتباه علامتش: DNS «could not be resolved» یا 403/502 بی‌دلیل. روش صحیح (رویهٔ §۵): از کارفرما بخواه پیش‌نمایش زندهٔ «CDP Browser Bridge Relay» را در رابط Arena باز کند؛ رکورد `page_loaded/page_heartbeat` در `relay-events.log` فوراً href واقعی را نشان می‌دهد — همان `$Base` است.
7. **(درس قبلی، اصلاح شد)** فرمت آدرس عمومی پلتفرم عوض شده است: `sbx-<id>.arena.site` به PowerShell پاسخ **403** می‌دهد (پروکسی جدید احراز می‌خواهد). فرمت فعلی پیش‌نمایش‌های زنده: **`https://{port}-{sandboxId}.e2b.app`** (مثل `https://8787-i6kbsthngkkzntvgolcy2.e2b.app`). قبل از اجرای اسکریپت پل، همیشه با حلقهٔ status pre-check (کد در همین سند §۵) مشخص کنید کدام Base کار می‌کند؛ مطمئن‌ترین منبع Base، URL پیش‌نمایش زندهٔ فرایند «CDP Browser Bridge Relay» در رابط Arena است. رلهٔ خود ایجنت هیچ‌وقت ۴۰۳ نمی‌دهد مگر کد جفت‌سازی غلط باشد (لاگ `down_count=0` یعنی درخواست اصلاً به رله نرسیده و ۴۰۳ از پروکسی است).
 (قاعدۀ ماندگار PowerShell کارفرما) و دستورهای کاربر همیشه PowerShell-native.
...
6. **بعد از ری‌بیلد سندباکس (مثلاً قطع برق)، URL پیش‌نمایش عوض می‌شود** — URL قدیمی `502 Bad Gateway` می‌دهد و URL جدید را نمی‌توان از `E2B_SANDBOX_ID` حدس زد. بازیابی: کارفرما پنل Live Preview را باز می‌کند؛ صفحهٔ دیاگ همان لحظه `href` تازه را به `/report` می‌فرستد؛ فقط `‎$Base` اسکریپت پل عوض می‌شود.
