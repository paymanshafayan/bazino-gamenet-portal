...
> **مرجع جدید دریافت شد — 2026-09-07:** [اصل طرح کاربر](INSTAGRAM_AFFILIATE_DESIGN.md) و [پلن نهایی پیشنهادی](INSTAGRAM_MEDIA_ONLY_PLAN.md) اکنون در ریپو هستند؛ مانع دریافت متن برطرف شد. Manus فقط media_id رسانهٔ منتشرشده را به `published-media` می‌فرستد؛ گیرنده، همان ID را با postId/mediaId رویداد Zernio تطبیق می‌دهد و کل گردش افیلیت در بک‌اند است. **کد فقط برای همکار، لینک خصوصی فقط برای دوست پس از Gate.** مطالب پایین شرح کد موجود/سابقه‌اند؛ فاز ۳ قرارداد جدید Manus نیست و هنوز کد آن اصلاح نشده است. مرجع ادامه: `HANDOFF_PROMPT.md §۱۹`.
...
> **تاریخچهٔ API موجود، نه جریان هدف جدید.** استفادهٔ Manus از این قرارداد و پرامپت ارسال لینک به همکار طبق پلن جدید باید متوقف/محدود شود؛ داده‌ها و لینک‌های تاریخی فقط با مهاجرت کنترل‌شده تغییر می‌کنند.
