...
> **جهت جدید ثبت‌شدهٔ کاربر — 2026-09-07:** Manus فقط شناسهٔ رسانهٔ منتشرشده را به `published-media` می‌فرستد؛ مسئولیت گردش افیلیت با بک‌اند است. بخش «فاز ۳» پایین، API موجود/سابقه است و نباید به‌عنوان قرارداد جدید Manus معرفی شود؛ در این نوبت کد آن حذف یا تغییر نکرده است. [پیش‌نویس و مرز دریافت سند](INSTAGRAM_MEDIA_ONLY_PLAN.md) و `HANDOFF_PROMPT.md §۱۸` را ببینید. اصل طرح پیوست تازه هنوز در محیط قابل خواندن نبوده و جزئیات آن از این سند حدس زده نمی‌شوند.
