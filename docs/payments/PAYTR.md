...
> وضعیت (به‌روزرسانی ۱۴۰۵/۰۶/۱۴): درگاه **پیاده‌سازی شده** (`server/payments/`) ولی به تصمیم کارفرما **پیش‌فرض خاموش** است (`PAYMENT_ONLINE_ENABLED` تنظیم نشده → همه‌ی مسیرها 503 و UI گزینه‌ی آنلاین را نشان نمی‌دهد). پرداخت‌های سایت فعلاً با **کیف پول / در محل** انجام می‌شود — [`WALLET.md`](./WALLET.md). جزئیات فنی API در [`paytr-api-reference.md`](./paytr-api-reference.md).
