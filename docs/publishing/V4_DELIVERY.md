...
## آخرین وضعیت تأییدشده — درخواست به‌روزرسانی همهٔ اسناد

- **کد آزموده‌شده:** `45dd2ad706015db4f41b6394f09c51976bafd5ef`؛ هر هشت بچ از قبل در همین شاخه و PR #20 وجود دارند. در این پیگیری کد محصول بازنویسی یا تغییر نکرد؛ اسناد با نتایج واقعیِ بازاجرای تکمیل‌شده به‌روز شدند.
- TypeScript سایت و مدیریت و build کامل production دوباره موفق؛ `npm test` **506/506 با صفر fail/skip**. هشت فایل JSON تازه و لاگ‌های `tests/reports/current-request-*` شاهدند، نه نتیجهٔ مانده از اجرای قبلی.
- Chromium production این بار **در همان اجرای اول** موفق شد: ۲۰ screenshot تمام‌صفحه، چهار زبان، دسکتاپ/موبایل، هر دو پنل، Composer و Gate. **هر ۲۰ تصویر با ابزار تصویر باز و بازبینی شدند**. restart واقعی حفظ فایل، metadata، Manus پیش‌فرض و خاموش‌بودن ارسال را تأیید کرد. رخداد timeout قدیمی در سابقه باقی است، نه نتیجهٔ این اجرا.
- PR #20 باز است؛ سه check موجود روی همین head موفق‌اند: run `34141810346`. این checkها جای ۵۰۶ تست محلی نیستند.
- تلاش تازهٔ `gh workflow run deploy.yml --ref arena/01a07a2f-bazino-gamenet-portal` باز هم **HTTP 403 / Resource not accessible by integration** داد. درخواست مستقیم health خطای TLS/EOF داشت؛ روش جایگزین fetch، HTML سایت را برگرداند، نه JSON با `service: bazino-zernio-receiver`. **استقرار production هنوز تأیید نشده است.**
- گزارش ماشینی: `tests/reports/current-request-summary.json`. این فایل و تصاویر/لاگ‌ها ignored هستند؛ با شواهد ZIP تحویل داده می‌شوند، نه در Git. در این نوبت هیچ محتوا، پیام، اعتبار سرویس یا پول واقعی مصرف نشد.

### کارهای باقی‌مانده — جدا از کد و تست محلی

1. **مجوز/انتشار هاست:** بررسی و merge کردن PR #20 توسط مدیر و اجرای مسیر مجاز Railway، یا اتصال مجدد GitHub در Arena با مجوز مناسب Actions/Workflows. push شاخه کار می‌کند؛ مجوز dispatch/تغییر workflow با آن یکی نیست. رمز یا توکن در چت لازم نیست.
2. **فعال‌سازی CI کامل:** قالب `publishing-v4.workflow.yml` موجود است، اما نصب آن در `.github/workflows/` نیازمند دسترسی مناسب است؛ workflow فعال قبلی این نوبت تغییر نکرده است.
3. **تنظیمات production:** تأیید Volume واقعی، `BAZINO_DATA_DIR`، حفظ JWT نصب قبلی، کلید اصلی `BAZINO_SECRETS_KEY` و تنظیمات مجاز Zernio/Manus از مسیر امن هاست/پنل.
4. **تست زنده پس از انتشار:** health باید JSON گیرنده باشد؛ سپس `webhook.test` امضاشده. PR/DM و انتشار واقعی پست فقط با حساب، کلید و approval محتوای مشخص تست شوند. نتیجهٔ mock به live تبدیل نمی‌شود.
5. **سیاست و دادهٔ واقعی:** تأیید شرایط کمپین/مالی/کوپن توسط مالک و بررسی صف پیش از روشن‌کردن ارسال؛ مبلغ fixture یا تیک خوداظهاری مجوز پرداخت/Follow قطعی نیست.
6. **محیط‌های بیرونی:** SQL Server و Mongo واقعی، Story/مجوز حساب و رفتار واقعی ارائه‌دهنده هنوز تست‌نشده‌اند. دادهٔ اصلی و شکل‌های ورودی پشتیبانی‌شده در راهنمای اجرا مشخص‌اند.
7. **متوقف تا دستور جدید:** SMTP و POS سخت‌افزاری؛ درگاه آنلاین خاموش می‌ماند. این‌ها به‌عنوان کار فعالِ این تحویل شروع نشده‌اند.
