...
class _HubScreenState extends State<HubScreen> {
  ...
  void _navigateToSection(String target) {
    // normalize hub.* and classic inner-page targets (games.detail, tournaments.weekly etc)
    final t = target.toLowerCase();
    setState(() {
      if (t == 'reserve' || t == 'games' || t == 'games.detail' || t == 'hub.games' || t == 'games.detail') {
        _currentIndex = 1;
      } else if (t == 'cafe' || t == 'cafe.detail' || t == 'cafe.cart' || t == 'hub.cafe') {
        _currentIndex = 2;
      } else if (t == 'shop' || t == 'shop.detail' || t == 'shop.cart' || t == 'hub.shop') {
        _currentIndex = 3;
      } else if (t == 'tournaments' || t == 'hub.tournaments' || t == 'tournaments.weekly' || t == 'weekly' || t == 'hub.weekly' || t == 'tournaments.special' || t == 'special' || t == 'hub.special' || t == 'tournaments.season' || t == 'season' || t == 'hub.season' || t == 'tournaments.brackets' || t == 'brackets' || t == 'hub.brackets' || t == 'tournaments.register' || t == 'register' || t == 'hub.register') {
        // map to tournament tab index
        if (t.contains('weekly')) {
          _tournamentInitialTab = 0;
        } else if (t.contains('special')) {
          _tournamentInitialTab = 1;
        } else if (t.contains('season')) {
          _tournamentInitialTab = 2;
        } else if (t.contains('bracket')) {
          _tournamentInitialTab = 3;
        } else if (t.contains('register')) {
          _tournamentInitialTab = 4;
        } else {
          _tournamentInitialTab = 0;
        }
        _currentIndex = 4;
      } else if (t == 'loyalty' || t == 'hub.loyalty' || t == 'club') {
        _currentIndex = 5;
      } else if (t == 'messages') {
        _currentIndex = 6;
      } else if (t == 'chat' || t == 'hub.chat') {
        _currentIndex = 7;
      } else if (t == 'blog' || t == 'blog.detail' || t == 'hub.blog') {
        _currentIndex = 8;
      } else {
        _currentIndex = 0;
      }
  ...
  Widget build(BuildContext context) {
    ...
    // Index 1: Reserves (games, games.detail)
    // Index 2: Cafe (cafe, cafe.detail, cafe.cart)
    // Index 3: Shop (shop, shop.detail, shop.cart)
    // Index 4: Tournament/Arena (tournaments.weekly/special/season/brackets/register + hub.* parity)
    ...
    final List<Widget> screens = [
      ...
      const CafeScreen(),
      const ShopScreen(),
      TournamentScreen(initialTab: _tournamentInitialTab),
      const AccountScreen(),
