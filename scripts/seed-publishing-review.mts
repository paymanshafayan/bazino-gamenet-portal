...
const store=new SqliteStore();await store.connect();await store.createDatabaseIfNotExist();if(!await store.countUsers())await store.seedMinimal({username:'admin',password:process.env.PREVIEW_PASSWORD!,email:'preview@example.test',phone:''});const core=new OpsCore(()=>store),service=new InstagramCampaignService(core);
...
if(!await store.getUserByUsername('review_partner'))await store.createUser({username:'review_partner',password:process.env.PREVIEW_PASSWORD!,email:'',phone:'+15555552002'});
await store.updateAffiliate('AFF-REVIEW',{username:'review_partner'});
...
fs.writeFileSync(`${process.env.PUBLISHING_REVIEW_DIR||'/home/user/.cache/bazino-v4'}/invite.json`,JSON.stringify({id,token:await service.linkToken(id,data)}),{mode:0o600});
