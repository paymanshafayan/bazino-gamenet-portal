...
export class InstagramCampaignService {
  ...
  async beforeSend(input:any){
    ...
    const unattended=String(input.memberId||'').match(/^(away|jarvis):/);
    if(unattended)return input.accountId===(await this.settings.config()).data.zernioAccountId;
  ...
  async afterSend(input:any,result:any){
    if(/^(away|jarvis):/.test(String(input.memberId||'')))return; // ig-inbox record already marks the reply
