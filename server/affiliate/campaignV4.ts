/** V4 state machine — NEW FLOW: partner comments keyword -> PR guide -> follow button -> DM private invite link.
 * Friend flow via comment code is retired. Friends register via partner's private link and get coupon.
 * Existing affiliate/member tables remain the financial/domain source.
 */
...
export class InstagramCampaignService {
  ...
  // NEW FLOW: only partner via keyword, friend via comment retired
  async onComment(e:InboxEvent):Promise<CampaignResult>{
    ...
    return this.core.store.runInTransaction(async()=>{
      ...
      const policy=eligible.campaign.data;
      const numeric=normalizeDigits((e.text||'').trim());

      // Friend flow via numeric code is retired in new design
      if(/^\d{6}$/.test(numeric)){
        return {ignored:true,reason:'friend_flow_retired'};
      }

      const lang=commentLanguage(e.text||'',policy,eligible.media.data.languages);
      ...
      const repeat=members.find(r=>r.data.role==='partner'&&r.data.igUserId===e.authorId&&r.data.mediaId===e.nativeId&&r.data.accountId===e.accountId&&r.data.campaignId===eligible.campaign.id);
      ...
      const aff:AffiliateRow={id:newId('AFF'),code,username:'',name:e.username||code,type:'instagram',language:lang,destination:'/',parentId:'',status:'active',newPct:-1,returnPct:0,tournamentPct:0,overridePct:0,notes:'Instagram v4 - new flow',createdAt:nowISO(),updatedAt:nowISO()};
      ...
      const m:CampaignMember={
        ...
        mediaId:e.nativeId!,
        campaignId:eligible.campaign.id,
        igUserId:e.authorId!,
        username:e.username||'',
        language:lang,
        partnerCode:code,
        parentId:'',
        status:'partner_follow_pending',
        policy,
        policyVersion:eligible.campaign.version,
        followMethod:'',
        followStatus:'unknown',
        shareStatus:'',
        buttonNonce:randomBytes(16).toString('hex'),
        createdAt:now,
        updatedAt:now
      };

      const legacy:IgMemberRow={id,role:'partner',campaignId:m.campaignId,mediaId:m.mediaId,commentId:e.commentId!,igUserId:m.igUserId,igUsername:m.username,partnerCode:code,parentMemberId:'',affiliateCode:code,status:m.status,followMethod:'',shareStatus:'',couponCode:'',inviteUrl:'',createdAt:now,updatedAt:now};
      await this.core.store.createIgMember(legacy);
      await this.saveMember(id,m,0);
      await this.core.save('pub-comment',commentKey,{memberId:id,campaignId:m.campaignId,language:lang},0);

      const template=m.policy.messages[lang].partner1;
      const text=renderCampaign(template,code);
      if(/invite_url|[?&](?:token|sig|gate)=|\/ig\/invite\//i.test(text))fail('PRIVATE_LINK_FORBIDDEN');
      ...
      const outboxId=await this.queue.enqueue({
        ...
        memberId:id,
        recipientId:m.igUserId,
        text,
        buttons:[{type:'postback',title:m.policy.messages[lang].button,payload:await this.button(id,m)}],
        expiresAt:new Date(Date.parse(e.createdAt!)+7*86400000).toISOString()
      },'partner_intro');

      await this.core.store.createIgEvent({id:newId('IGE'),memberId:id,mediaId:m.mediaId,commentId:e.commentId!,kind:'partner_comment',payload:'',result:'queued',verificationMethod:'comment_id',createdAt:now});
  ...
  async onMessage(e:InboxEvent):Promise<CampaignResult>{
    ...
    // New flow only supports partner
    if(member.data.role!=='partner')return {ignored:true,reason:'friend_flow_retired'};
    if(!['partner_follow_pending'].includes(member.data.status))return {ignored:true,duplicate:true};
    if(!await this.registry.eligible(e.accountId,member.data.mediaId))return {ignored:true};

    // External lookup outside DB transaction
    const follow=await this.queue.client.follow(e.accountId,e.authorId);

    return this.core.store.runInTransaction(async()=>{
      const row=await this.core.read<CampaignMember>('pub-member',member.id);if(!row||!['partner_follow_pending'].includes(row.data.status))return {ignored:true};
      const m={...row.data,followStatus:follow,followMethod:follow==='verified'?'follow_verified':'button_event_only',conversationId:e.conversationId,lastInteractionAt:e.timestamp,updatedAt:nowISO()} as CampaignMember;
      if(follow==='not_following'||m.policy.requireVerifiedFollow&&follow!=='verified'){
        await this.saveMember(row.id,m,row.version);
        return {ignored:true,reason:'follow_pending'};
      }
      // Partner now gets private invite link
      const stage='partner_link';
      m.status='link_ready';
      // Link valid 365 days for partner to share unlimited times
      m.linkExpiresAt=new Date(Date.now()+365*86400000).toISOString();
      await this.saveMember(row.id,m,row.version);

      const text=m.policy.messages[m.language].partner2;
      // partner2 IS allowed to contain invite_url placeholder — will be replaced in prepareMessage
      ...
      const outboxId=await this.queue.enqueue({
        ...
        text,
        buttons:[],
        expiresAt:new Date(Date.parse(e.timestamp)+24*3600000).toISOString()
      },stage);
  ...
  // Supports both partner (new flow) and friend (legacy)
  async linkToken(id:string,m:CampaignMember){
    const key=await this.settings.invitationKey();
    if(m.role==='partner'){
      // Partner link is reusable, bound to partner member, with optional expiry
      const base=m.linkExpiresAt ? `partner:${id}:${m.igUserId}:${m.campaignId}:${m.mediaId}:${m.linkExpiresAt}` : `partner:${id}:${m.igUserId}:${m.campaignId}:${m.mediaId}`;
      return createHmac('sha256',key).update(base).digest('hex');
    }
    if(m.role==='friend'){
      if(!m.linkExpiresAt)fail('FRIEND_GATE_REQUIRED',409);
      return createHmac('sha256',key).update(`friend:${id}:${m.igUserId}:${m.campaignId}:${m.mediaId}:${m.linkExpiresAt}`).digest('hex');
    }
    fail('FRIEND_GATE_REQUIRED',409);
  }

  async verifyLink(id:string,token:string){
    const m=await this.core.read<CampaignMember>('pub-member',id);
    if(!m)fail('INVITE_INVALID_OR_EXPIRED',404);

    // Partner flow: reusable link
    if(m.data.role==='partner'){
      const allowed=['link_ready','link_sent','code_ready','code_sent','activated'];
      if(!allowed.includes(m.data.status))fail('INVITE_INVALID_OR_EXPIRED',404);
      if(m.data.linkExpiresAt && Date.parse(m.data.linkExpiresAt)<Date.now())fail('INVITE_INVALID_OR_EXPIRED',404);
      if(!sameSig(token,await this.linkToken(id,m.data)))fail('INVITE_INVALID_OR_EXPIRED',404);
      return m;
    }

    // Legacy friend flow (kept for backward compatibility, but new comments won't create friends)
    if(m.data.role==='friend'){
      if(!['link_ready','link_sent','activated'].includes(m.data.status)||!m.data.linkExpiresAt||Date.parse(m.data.linkExpiresAt)<Date.now())fail('INVITE_INVALID_OR_EXPIRED',404);
      if(!sameSig(token,await this.linkToken(id,m.data)))fail('INVITE_INVALID_OR_EXPIRED',404);
      return m;
    }

    fail('INVITE_INVALID_OR_EXPIRED',404);
  }

  async prepareMessage(input:any){
    const cfg=await this.settings.config();
    if(input.stage==='partner_link' || input.stage==='partner_code'){
      const r=await this.core.read<CampaignMember>('pub-member',input.memberId);
      if(!r||r.data.role!=='partner'||r.data.igUserId!==input.recipientId)fail('FRIEND_GATE_REQUIRED',409);
      const token=await this.linkToken(r.id,r.data);
  ...
  async afterSend(input:any,result:any){
    ...
    if(input.stage==='partner_link' || input.stage==='partner_code') status='link_sent';
    else if(input.stage==='friend_link') status='link_sent';
    // partner_intro keeps partner_follow_pending until button tap
