...
export function buildSkillRegistry(): Skill[] {
  ...
  /* ── NEW: Tournament creation (user requested Jarvis can create tournament) ── */
  skills.push({
    id: 'create_tournament', title: 'ساخت تورنمنت جدید', risk: 'sensitive',
    description: 'Create a new tournament: title, game (Valorant/CS2/Dota2), fee, startDate, maxTeams. Goes to approval queue.',
    params: obj({
      title: str('Tournament title, e.g. مسابقات Valorant قهرمانی'),
      game: str('Game name, e.g. Valorant, CS2, Dota2'),
      registrationFee: num('Registration fee in Toman'),
      startDate: str('Start date YYYY-MM-DD or Persian date'),
      maxTeams: num('Max teams, e.g. 8 or 16'),
      status: str('Status: Upcoming/Active (optional)'),
    }, ['title', 'game']),
    run: async (ctx, input) => {
      const title = String(input?.title || '').trim().slice(0, 200);
      const game = String(input?.game || '').trim().slice(0, 80) || 'Valorant';
      if (!title) return ERR('عنوان تورنمنت خالی است');
      const id = `trn-${randomUUID().slice(0, 6)}`;
      const newTour = {
        id,
        title,
        game,
        registrationFee: Number.isFinite(Number(input?.registrationFee)) ? Number(input.registrationFee) : 0,
        startDate: String(input?.startDate || new Date().toISOString().slice(0, 10)).slice(0, 50),
        maxTeams: Number.isFinite(Number(input?.maxTeams)) ? Math.max(2, Math.min(64, Number(input.maxTeams))) : 8,
        status: String(input?.status || 'Upcoming').slice(0, 20),
        registeredTeamsCount: 0,
        teams: '[]',
        bracket: JSON.stringify({ round1: [], semis: [], finals: [] }),
      };
      await ctx.store.createTournament(newTour);
      return OK(`تورنمنت «${title}» (${game}) ساخته شد — ${newTour.maxTeams} تیم، شروع ${newTour.startDate}`, { tournamentId: id, tournament: newTour });
    },
  });

  /* ── NEW: Blog article creation ── */
  skills.push({
    id: 'create_article', title: 'ساخت مقاله بلاگ', risk: 'sensitive',
    description: 'Create a blog article directly (news, tournament announcement). Goes to approval queue.',
    params: obj({
      title: str('Article title'),
      content: str('Article body (markdown allowed)'),
      category: str('Category e.g. Tournaments, News, Tutorial'),
      imageUrl: str('Image URL optional'),
    }, ['title', 'content']),
    run: async (ctx, input) => {
      const title = String(input?.title || '').trim().slice(0, 200);
      const content = String(input?.content || '').trim().slice(0, 8000);
      if (!title || !content) return ERR('عنوان یا محتوا خالی است');
      const id = `art-${randomUUID().slice(0, 6)}`;
      const newArt = {
        id,
        title,
        content,
        category: String(input?.category || 'News').slice(0, 80),
        imageUrl: String(input?.imageUrl || '/images/home/esports-480.webp').slice(0, 500),
        mobileImageUrl: '',
        author: 'جارویس',
        date: new Date().toLocaleDateString('fa-IR'),
        comments: '[]',
      };
      await ctx.store.createArticle(newArt);
      return OK(`مقاله «${title}» ساخته شد`, { articleId: id });
    },
  });

  /* ── NEW: App Slider management ── */
  skills.push({
    id: 'list_app_sliders', title: 'لیست اسلایدرها', risk: 'read',
    description: 'List home/app slider banners with image, title, target link.',
    params: obj({}),
    run: async (ctx) => {
      const rows = await ctx.store.listSliders();
      return OK(`${rows.length} اسلایدر: ${cap(rows, 10).map((s: any) => `${s.titleFa || s.titleEn} → ${s.target}`).join(' · ')}`, { items: rows });
    },
  });

  skills.push({
    id: 'create_app_slider', title: 'ساخت اسلایدر جدید', risk: 'sensitive',
    description: 'Create a new home/app slider banner: imageUrl, target (e.g. /tournaments), titles. For changing site images.',
    params: obj({
      imageUrl: str('Image URL, e.g. https://... or /images/...'),
      target: str('Click target, e.g. /tournaments, /cafe, /reserve'),
      titleFa: str('Persian title'),
      titleEn: str('English title'),
      descFa: str('Persian description optional'),
    }, ['imageUrl', 'target']),
    run: async (ctx, input) => {
      const imageUrl = String(input?.imageUrl || '').trim().slice(0, 500);
      const target = String(input?.target || '/').trim().slice(0, 200) || '/';
      if (!imageUrl) return ERR('آدرس تصویر خالی است');
      const id = `slide-${randomUUID().slice(0, 6)}`;
      const newSlide = {
        id,
        imageUrl,
        mobileImageUrl: '',
        target,
        titleFa: String(input?.titleFa || '').slice(0, 200),
        titleEn: String(input?.titleEn || '').slice(0, 200),
        titleRu: String(input?.titleRu || '').slice(0, 200),
        titleTr: String(input?.titleTr || '').slice(0, 200),
        descFa: String(input?.descFa || '').slice(0, 500),
        descEn: String(input?.descEn || '').slice(0, 500),
        descRu: '',
        descTr: '',
      };
      await ctx.store.createSlider(newSlide);
      return OK(`اسلایدر «${newSlide.titleFa || newSlide.titleEn}» ساخته شد → ${target}`, { sliderId: id, slider: newSlide });
    },
  });

  skills.push({
    id: 'delete_app_slider', title: 'حذف اسلایدر', risk: 'sensitive',
    description: 'Delete a slider by id.',
    params: obj({ id: str('Slider id') }, ['id']),
    run: async (ctx, input) => {
      const id = String(input?.id || '').trim();
      if (!id) return ERR('شناسه اسلایدر خالی است');
      await ctx.store.deleteSlider(id);
      return OK(`اسلایدر ${id} حذف شد`, { sliderId: id });
    },
  });
...
export const SENSITIVE_IDS = ['adjust_credits', 'update_cafe_item', 'create_coupon', 'answer_ticket', 'ig_away_toggle', 'publish_content', 'send_ig_reply', 'create_tournament', 'create_article', 'create_app_slider', 'delete_app_slider'];
