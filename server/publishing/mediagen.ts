...
export class MediaGenService {
  ...
  async generate(actor: string, b: any) {
    ...
    const provider = b.provider === 'flux' ? 'flux' : b.provider === 'imejis' ? 'imejis' : fail('PROVIDER_REQUIRED');
    ...
    const key = fingerprint({ actor, provider, designId, fields, prompt, title, language });
  ...
  private async saveAsset(task: MediaGenTask, mime: string, bytes: Buffer): Promise<string> {
    const created = await this.assets.create(task.owner, {
      mime, size: bytes.length, name: `mediagen-${task.provider}-${task.designId || 'flux'}.${mime === 'image/png' ? 'png' : 'jpg'}`,
      idempotencyKey: `mediagen:${fingerprint({ task: task.createdAt, provider: task.provider, designId: task.designId, prompt: task.prompt })}`,
    });
    const id = created.id;
  ...
  async import(actor: string, id: string, b: any) {
    ...
    const format = b.format === 'story' ? 'story' : 'image';
    ...
    const min = (format === 'story' ? 9 / 16 : 0.8) - 0.01, max = 1.91 + 0.01;
