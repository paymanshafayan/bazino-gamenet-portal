...
export default function CafeTab({
  ...
  const handleApplyCoupon = () => {
    ...
    addNotification(L(language, { fa: 'تخفیف بوفه با موفقیت اعمال شد!', en: 'Buffet discount applied successfully!', ru: 'Скидка на буфет успешно применена!', tr: 'Büfe indirimi başarıyla uygulandı!' }), 'success');
  };
  ...
  const handleCheckout = async () => {
    ...
    if (!systemNumber.trim()) {
      addNotification(L(language, { fa: 'لطفاً شماره سیستم یا صندلی خود را وارد کنید.', en: 'Please enter your system or seat number.', ru: 'Пожалуйста, введите номер места или системы.', tr: 'Lütfen sistem veya koltuk numaranızı girin.' }), 'error');
      return;
    }
    ...
    <>
      <div className="rounded-2xl p-5 relative overflow-hidden bg-dark-card border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2 relative z-10 font-display">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2.5 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer uppercase tracking-wider border ${selectedCategory === cat ? 'bg-primary text-black border-primary shadow-[0_0_15px_rgba(0,240,255,0.25)]' : 'text-gray-400 hover:text-white bg-card-3 hover:bg-white/5 border-white/10'}`}
            >
        ...
        <div className="text-xs text-gray-400 flex items-center gap-2 relative z-10 font-medium">
          ...
          <span>{L(language, { fa: 'سفارش بوفه مستقیماً روی صندلی یا میز بازی شما تحویل داده می‌شود', en: 'Buffet orders are delivered directly to your system gaming desk', ru: 'Заказы из буфета доставляются прямо к вашему игровому столу', tr: 'Büfe siparişleri doğrudan oyun masanıza teslim edilir' })}</span>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredItems.map((item) => {
          ...
          if (language !== 'fa') {
            if (item.name.includes('پیتزا پپرونی')) translatedName = language === 'en' ? 'Pepperoni Pizza XL' : language === 'ru' ? 'Пицца Пепперони XL' : 'Pepperoni Pizza XL';
            else if (item.name.includes('همبرگر دوبل')) translatedName = language === 'en' ? 'Double Cheese Burger' : language === 'ru' ? 'Двойной Чизбургер' : 'Double Burger';
            else if (item.name.includes('سیب‌زمینی سرخ‌کرده')) translatedName = language === 'en' ? 'French Fries XL' : language === 'ru' ? 'Картофель Фри XL' : 'Patates Kızartması XL';
            else if (item.name.includes('انرژی‌زا ردبول')) translatedName = language === 'en' ? 'RedBull Energy Drink' : language === 'ru' ? 'Энергетик RedBull' : 'RedBull Enerji İçeceği';
            else if (item.name.includes('موهیتو خنک')) translatedName = language === 'en' ? 'Fresh Mojito Ice' : language === 'ru' ? 'Ледяной Мохито' : 'Nane Limon Mojito';
            else if (item.name.includes('قهوه اسپرسو دوبل')) translatedName = language === 'en' ? 'Double Espresso Shot' : language === 'ru' ? 'Двойной Эспрессо' : 'Double Espresso';
            else if (item.name.includes('چیپس ساده نمکی')) translatedName = language === 'en' ? 'Salted Potato Chips' : language === 'ru' ? 'Чипсы соленые' : 'Sade Tuzlu Cips';
            else if (item.name.includes('پفک نمکی لینا')) translatedName = language === 'en' ? 'Lina Crunchy Snacks' : language === 'ru' ? 'Кукурузные Снеки' : 'Lina Çıtır Atıştırmalık';
          }
          return (
            <div key={item.id} className="rounded-2xl border border-white/10 bg-dark-card overflow-hidden flex flex-col group hover:border-primary/50 hover:shadow-[0_0_20px_rgba(0,240,255,0.1)] transition-all duration-300">
              <div className="relative aspect-video w-full bg-card-2 overflow-hidden">
                <img loading="lazy" src={item.imageUrl} alt={translatedName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer" />
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex justify-between items-center mt-2.5 text-xs text-gray-500 font-medium">
                    <span>{t('cafe.inventory', 'موجودی:')}: <strong className="text-gray-300 font-mono font-bold">{item.inventory}</strong> {t('cafe.pcs', 'عدد')}</span>
                    <span className="text-primary font-mono text-[10px] font-bold uppercase tracking-wider">{L(language, { fa: 'تحویل روی سیستم', en: 'Seat Delivery', ru: 'К месту', tr: 'Masaya Hizmet' })}</span>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-gray-500 block font-bold font-mono uppercase">{L(language, { fa: 'قیمت:', en: 'Price:', ru: 'Цена:', tr: 'Fiyat:' })}</span>
                  ...
                  <button onClick={() => addToCart(item)} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg border border-primary text-primary font-black text-[10px] uppercase tracking-wider hover:bg-primary hover:text-black transition-all font-display cursor-pointer">
      ...
      {cart.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-card-2 p-3.5 rounded-xl border border-white/5">
            <label className="text-xs text-gray-400 block mb-1.5 font-bold">{L(language, { fa: 'تحویل روی کدام صندلی/میز؟', en: 'Deliver to which seat/desk?', ru: 'Доставить к какому месту?', tr: 'Hangi masaya teslim edilsin?' })}</label>
            <select value={systemNumber} onChange={(e) => setSystemNumber(e.target.value)} className="w-full bg-card-2 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-primary font-bold cursor-pointer font-mono">
              <option value="Seat #01">Seat #01</option>
              <option value="Seat #05">Seat #05</option>
              <option value="Table VIP #1">Table VIP #1</option>
            </select>
          </div>
          ...
          <div className="border-t border-white/5 pt-4">
            {appliedCoupon ? (
              <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 p-2.5 rounded-lg text-xs font-mono">
                <div className="flex items-center gap-1.5 text-emerald-400 font-bold"><Check className="w-4 h-4 animate-pulse" /><span>{L(language, { fa: 'اعمال شد:', en: 'Applied:', ru: 'Применено:', tr: 'Uygulandı:' })} {appliedCoupon.code}</span></div>
          ...
          <div className="border-t border-white/5 pt-4 space-y-2.5 text-xs text-gray-400 font-mono">
            <div className="flex justify-between font-medium"><span>{L(language, { fa: 'مجموع اقلام بوفه:', en: 'Items Subtotal:', ru: 'Подитог товаров:', tr: 'Yiyecekler Toplamı:' })}</span><span className="text-gray-200">{subtotal.toLocaleString(localeOf(language))} {t('common.currency', 'لیر')}</span></div>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-3.5 text-primary mt-2 relative overflow-hidden font-sans">
              ...
              <span className="relative z-10 block text-[10px] leading-relaxed text-gray-400 font-medium font-sans">{L(language, { fa: `با تکمیل این سفارش، ${Math.floor(total / 10)} امتیاز دریافت خواهید کرد.`, en: `By confirming this order, you will earn ${Math.floor(total / 10)} points.`, ru: `После оплаты заказа вам начислится ${Math.floor(total / 10)} баллов.`, tr: `Bu siparişi tamamladığınızda ${Math.floor(total / 10)} puan kazanacaksınız.` })}</span>
            </div>
          </div>
          <button onClick={handleCheckout} disabled={isSubmitting} className="w-full mt-2 py-4 bg-primary text-black font-black uppercase tracking-wider rounded-lg shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:bg-primary-hover border-2 border-primary transition-all flex items-center justify-center gap-2 cursor-pointer font-display text-xs disabled:opacity-60 disabled:cursor-not-allowed">
            <ShoppingCart className="w-4 h-4" /><span>{t('cafe.btnOrder', 'ثبت نهایی سفارش بوفه')}</span>
          </button>
        </div>
      )}
    </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-fade-in font-sans" dir={dir}>
      ...
      {checkout && <CheckoutModal kind="cafe" params={checkout.params} estimatedAmount={checkout.amount} onClose={() => setCheckout(null)} onDone={(r: CheckoutResult) => { setCheckout(null); addNotification(L(language, { fa: `سفارش شما ثبت شد (${r.orderId}). هزینه هنگام تحویل روی صندلی ${systemNumber} در محل تسویه می‌شود و امتیاز پس از پرداخت به حساب‌تان می‌آید.`, en: `Order registered (${r.orderId}). Pay at the venue on delivery to seat ${systemNumber}; points are credited after payment.`, ru: `Заказ оформлен (${r.orderId}). Оплата на месте при доставке к месту ${systemNumber}; баллы начисляются после оплаты.`, tr: `Sipariş kaydedildi (${r.orderId}). ${systemNumber} numaralı koltuğa teslimatta mekânda ödenir; puanlar ödemeden sonra eklenir.` }), 'success'); setCart([]); setAppliedCoupon(null); setCouponCode(''); }} />}
