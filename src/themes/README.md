...
- **بخش ۱ (Hero) و بخش ۱۲ (CTA) اجباری‌اند** — بدون آن‌ها قالب ناقص است.
- بخش‌های ۲ تا ۷ **توصیه‌شده** (هسته‌ی محتوای کلوپ گیمینگ).
- بخش‌های ۸ تا ۱۱ **اختیاری** (بسته به سبک قالب).
...
| # | بخش | توضیح | منبع داده |
|---|---|---|---|
| ۱ | **Hero / اسلایدر اصلی** ⭐ اجباری | اسلایدهای بازی‌های ویژه: تصویر، عنوان، توضیح، دکمه CTA | `featuredGames` |
| ۲ | **ژانرهای بازی** | گرید دسته‌بندی بازی‌ها (شوتر، مسابقه‌ای، نقش‌آفرینی و...) | `gameGenres` |
| ۳ | **سالن‌ها و خدمات** | معرفی سالن‌ها/زون‌های کلوپ (VIP، کافه، فروشگاه و...) | `loungeSections` |
| ۴ | **نتایج مسابقات** | برد نتایج اخیر مسابقات (اسکوربورد) | `matchHistory` |
| ۵ | **تورنمنت‌های فعال** | کاروسل/گرید تورنمنت‌ها با وضعیت، تصویر و جایزه | `tournaments` |
| ۶ | **تعرفه‌ها / پاس‌ها** | پلن‌های قیمت‌گذاری (ساعتی، روزانه، VIP) | `pricingPackages` |
| ۷ | **مربیان و تیم** | معرفی کادر/مربیان | `staffTeam` |
| ۸ | **چرا ما + آمار** | امتیازات کلوپ + آمار (اختیاری) | `settings` |
| ۹ | **درباره کلوپ** | داستان/معرفی کلوپ (اختیاری) | `settings` |
| ۱۰ | **محصولات ویژه** | ویترین فروشگاه (اختیاری) | `settings`/داده‌ها |
| ۱۱ | **تماس، آدرس و نقشه** | اطلاعات تماس، شبکه‌های اجتماعی، نقشه (اختیاری) | `settings` |
| ۱۲ | **CTA های ناوبری** ⭐ اجباری | دکمه‌های رفتن به رزرو/کافه/فروشگاه/مسابقات | `onNavigate` |
    ...
    return {
      ...
      render: function (props) {
        ...
        return R.createElement('div', { className: 'my-theme-home' },
          /* بخش ۱ — Hero */
          R.createElement('section', { className: 'hero' },
            R.createElement('h1', null, (props.settings && props.settings.clubName) || 'BAZINO'),
            R.createElement('button', { onClick: function () { props.onNavigate('reservations'); } },
              props.language === 'fa' ? 'رزرو' : 'Reserve')
          )
          /* ... بخش‌های ۲ تا ۱۲ طبق جدول بالا ... */
        );
      }
    };
  });
