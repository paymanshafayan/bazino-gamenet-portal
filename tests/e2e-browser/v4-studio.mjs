...
for(const [lang,width] of [['fa',1440],['en',1440],['tr',390],['ru',390]]){
 ...
 if(['fa','en'].includes(lang))await page.screenshot({path:`/home/user/visual-testing/v4/studio-overview-${lang}.png`,fullPage:true});
 ...
 if(['fa','ru'].includes(lang))await page.screenshot({path:`/home/user/visual-testing/v4/studio-agents-${lang}.png`,fullPage:true});
 await page.locator('[data-pub-tab=settings]').click();if(['fa','tr'].includes(lang))await page.screenshot({path:`/home/user/visual-testing/v4/studio-settings-${lang}.png`,fullPage:true});
 await page.locator('[data-pub-tab=campaigns]').click();await page.locator('.pub-card-grid .pub-secondary').first().click();await page.locator('[role=dialog]').waitFor();if(['fa','tr'].includes(lang)){await page.evaluate(()=>window.scrollTo(0,0));await page.setViewportSize({width,height:1800});await page.screenshot({path:`/home/user/visual-testing/v4/studio-campaign-${lang}-full.png`,fullPage:true});await page.setViewportSize({width,height:1000});}await page.keyboard.press('Escape');
...
writeFileSync('/home/user/visual-testing/v4/studio-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
